...
// Almost identical to RtComPhoton except we randomly select light subpaths
// This adhoc implementation is only for experimental purpose and yield slower result than RtComPhoton (due to non coalescent access of light subpaths)
// The only method that was tested is VplPhoton.
class RtLvcComPhoton: public RtTechnique
{
public:
	...
	void render(shared_ptr<RtScene> & scene, const glm::vec2 & resolution, const nlohmann::json & json) override
	{
		...
		mFrameMode = RtLvcComPhoton::EFrameModeStrMap.at(json["frameMode"]);
		if(json.find("misMode") == json.end())
		{
			mMisMode = RtLvcComPhoton::EMis::Balance;
		}
		else
		{
			mMisMode = RtLvcComPhoton::EMisModeStrMap.at(json["misMode"]);
		}
	...
	void initOptixRtMaterial()
	{
		try
		{
			...
			mOptixRtMaterial->setClosestHitProgram(0, mOptixContext->createProgramFromPTXFile("ptxfiles/reflectcuts_generated_lvclighttracing.cu.ptx", "rtMaterialClosestHit"));
			mOptixRtMaterial->setAnyHitProgram(1, mOptixContext->createProgramFromPTXFile("ptxfiles/reflectcuts_generated_lvclighttracing.cu.ptx", "rtMaterialAnyHit"));
		}
	...
	void initOptixVplProgram()
	{
		...
		try
		{
			...
			// create vpl program
			mOptixPtProgram = mOptixContext->createProgramFromPTXFile("ptxfiles/reflectcuts_generated_lvclighttracing.cu.ptx", "splatColor");

			mOptixContext->setRayGenerationProgram(EOptixPasses::VplSplat, mOptixPtProgram);

			optix::Program exceptionProgram = mOptixContext->createProgramFromPTXFile("ptxfiles/reflectcuts_generated_lvclighttracing.cu.ptx", "exception");
...
//			_getch();
	...
	void initOptixLightTracingProgram()
	{
		...
		try
		{
			...
			mOptixLightTracingProgram = mOptixContext->createProgramFromPTXFile("ptxfiles/reflectcuts_generated_lvclighttracing.cu.ptx", "tracePhotons");
...
const std::map<std::string, RtLvcComPhoton::EFrame> RtLvcComPhoton::EFrameModeStrMap = {
	{"accumulate", RtLvcComPhoton::EFrame::Accumulate},
	{"cleareveryframe", RtLvcComPhoton::EFrame::ClearEveryFrame}
};

const std::map<std::string, RtLvcComPhoton::EMis> RtLvcComPhoton::EMisModeStrMap = {
	{"one", RtLvcComPhoton::EMis::One},
	{"balance", RtLvcComPhoton::EMis::Balance},
	{"max", RtLvcComPhoton::EMis::Max},
	{"power2", RtLvcComPhoton::EMis::Power2},
	{"geometryClamp", RtLvcComPhoton::EMis::GeometryClamp},
	{"geometryBrdfClamp", RtLvcComPhoton::EMis::GeometryBrdfClamp}
};